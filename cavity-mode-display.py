...
- R1, R2, and L controls use a 1000 mm span with exact-value entry.
...
def make_light_palette() -> QPalette:
    """Return a palette that keeps Qt widgets in a light theme."""
    palette = QPalette()
    palette.setColor(QPalette.ColorRole.Window, QColor("#f7f8fb"))
    ...
    palette.setColor(QPalette.ColorRole.Button, QColor("#f7f8fb"))
    palette.setColor(QPalette.ColorRole.ButtonText, QColor("#1f2933"))
    palette.setColor(QPalette.ColorRole.BrightText, QColor("#ffffff"))
    palette.setColor(QPalette.ColorRole.Highlight, QColor("#2f6fde"))
    palette.setColor(QPalette.ColorRole.HighlightedText, QColor("#ffffff"))
    palette.setColor(QPalette.ColorRole.Link, QColor("#2f6fde"))
    palette.setColor(QPalette.ColorRole.LinkVisited, QColor("#275cbc"))
    palette.setColor(QPalette.ColorGroup.Disabled, QPalette.ColorRole.Text, QColor("#7a8593"))
    palette.setColor(QPalette.ColorGroup.Disabled, QPalette.ColorRole.ButtonText, QColor("#7a8593"))
    palette.setColor(QPalette.ColorGroup.Disabled, QPalette.ColorRole.WindowText, QColor("#7a8593"))
    return palette
...
class CenteredRangeSlider(QWidget):
    """Slider with exact-value entry and a fixed span in mm."""
    ...
    def __init__(
        ...
        self.value_spin.setRange(1.0, 1_000_000.0)
        self.value_spin.setDecimals(0)
        self.value_spin.setSingleStep(1.0)
        self.value_spin.setMinimumWidth(90)
        ...
        self.value_spin.valueChanged.connect(self._on_value_spin_changed)

        self._set_center(value_mm)
    ...
    def set_value_mm(self, value_mm: float, emit: bool = True) -> None:
        if not (self._min_mm <= value_mm <= self._max_mm):
            self._set_center(value_mm)
        val = int(round(np.clip(value_mm, self._min_mm, self._max_mm)))
        with QSignalBlocker(self.slider), QSignalBlocker(self.value_spin):
    ...
    def _on_slider_changed(self, value_mm: int) -> None:
        with QSignalBlocker(self.value_spin):
            self.value_spin.setValue(float(value_mm))
        self.value_changed.emit(float(value_mm))

    def _on_value_spin_changed(self, value_mm: float) -> None:
        self.set_value_mm(float(value_mm), emit=True)
...
class CavityMainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.ctrl_r1 = CenteredRangeSlider("R1", value_mm=500.0, span_mm=1000.0)
        self.ctrl_r2 = CenteredRangeSlider("R2", value_mm=500.0, span_mm=1000.0)
        self.ctrl_l = CenteredRangeSlider("Length L", value_mm=400.0, span_mm=1000.0)
    ...
    def update_plot(self, _value: float | None = None) -> None:
        ...
        self.figure.suptitle("Fabry-Perot Cavity Interactive View", fontsize=13, weight="bold")
    ...
    def _draw_panel_1(
        ...
        if mode is not None:
            ...
            ax.text(
                0.5,
                0.98,
                f"Waist position: {z_w_mm:.2f} mm    Waist radius: {mode['w0'] * mm:.4f} mm",
                transform=ax.transAxes,
                ha="center",
                va="top",
                fontsize=10,
                color="#1f2933",
                bbox={"boxstyle": "round,pad=0.25", "facecolor": "#ffffff", "edgecolor": "#d3dbe6", "alpha": 0.9},
            )
        elif error_text:
            ax.text(
                0.5,
                ...
                fontsize=10,
                color="firebrick",
                bbox={"boxstyle": "round,pad=0.25", "facecolor": "#ffffff", "edgecolor": "#e3b3b3", "alpha": 0.9},
            )

        mirror_h = max(0.05, y_max_mm)
        mirror_w = max(0.6, 0.02 * L_mm)

        self._add_rectangular_mirror(
            ax=ax,
            z_vertex_mm=0.0,
        ...
        ax.set_title("Cavity Mode Profile (side view)")
        ax.set_xlim(-pad_mm, L_mm + pad_mm)
        ax.set_ylim(-y_lim, y_lim)
        ax.grid(True, alpha=0.28)

    def _add_rectangular_mirror(
        self,
        ax,
        z_vertex_mm: float,
    ...
    def _draw_panel_4(self, g1: float, g2: float) -> None:
        ...
        status_text, status_color = self._stability_label(g1, g2)
        ax.text(
            0.5,
            0.08,
            status_text,
        ...
        ax.set_title("Cavity Stability Diagram")
    ...
    def _stability_label(self, g1: float, g2: float) -> tuple[str, str]:
        special_points = (
            ("CONFOCAL", 0.0, 0.0, "darkgreen"),
            ("CONCENTRIC", -1.0, -1.0, "#8a5a00"),
            ("PLANAR", 1.0, 1.0, "#8a5a00"),
        )
        for label, g1_target, g2_target, color in special_points:
            if np.isclose(g1, g1_target, atol=1e-3) and np.isclose(g2, g2_target, atol=1e-3):
                return label, color

        stable = 0.0 <= g1 * g2 <= 1.0
        return (
            ("STABLE", "darkgreen") if stable else ("UNSTABLE", "firebrick")
        )
